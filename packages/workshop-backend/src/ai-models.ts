...
function getModelViaGateway(
  ...
  // Binding transport when available (all providers except Google): the model's baseUrl names
  // the gateway's provider passthrough on the binding's own host and env.WORKERS_AI.fetch()
  // serves it, pre-authenticated in-account. pi's API impls require a recognized auth header
  // before dispatch, so binding-routed requests carry a sentinel cf-aig-authorization, which
  // the gateway ignores and strips.
